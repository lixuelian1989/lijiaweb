 ...
 * @version  $Id: MemberAction.class.php 2791 2013/12/10  yangshuai $
 +------------------------------------------------------------------------------
 */
class TMemberAction extends CommonAction {
	...
	public function scenics($field){
		...
		$s_model=D('Scenic');
		$scenics=$s_model->field('`id`,`sce_name`,`sce_img`,`sce_count`,`sce_detail`,`sce_grade`')->where(array('status'=>1))->order('sce_count desc')->select();
		if(!empty($scenics)){
			foreach ($scenics as $k=>$v){
				$scenics[$k]['sce_img']=empty($v['sce_img'])?'':SITE_URL.$v['sce_img'];
				$scenics[$k]['sce_detail']=empty($v['sce_detail'])?'':msubstr($v['sce_detail'], $start=0, 20, $charset="utf-8", $suffix=true);
...
public function listen($field){
		...
		if($id){
			$s_model=D('Scenic');
			$scenics=$s_model->field('id,sce_voice,sce_detail,sce_famous,sce_product,sce_images')->find($id);
			if(!empty($scenics)){
				//点击收听 景区收听次数+1
		...
		if($view_id){
			$v_model=D('View');
			$views=$v_model->field('sce_id id,view_voice sce_voice,view_famous sce_famous,view_detail sce_detail,view_product sce_product,view_images sce_images')->find($view_id);
			if(!empty($views)){
				//点击收听 景区收听次数+1
				$v_model->where('id='.$id)->setInc('view_count');
				//插入我的足迹表
				if(!D('Log')->where(array('uid'=>$uid,'sce_id'=>$views['id']))->find())
					//保存
					D('Log')->add(array('uid'=>$uid,'sce_id'=>$views['id']));
				
				$views['sce_voice']=empty($views['sce_voice'])?'':SITE_URL.$views['sce_voice'];
				$views['sce_detail']=empty($views['sce_detail'])?'':$views['sce_detail'];
				$views['sce_famous']=empty($views['sce_famous'])?'':$views['sce_famous'];
				$views['sce_product']=empty($views['sce_product'])?'':$views['sce_product'];
...
//最近景区
	...
	public function register($field){
		...
		$where['email']=$email;
		$where['_logic'] = 'or';
		$member=D('member')->where($where)->count();
		//用户名已经存在
		if($member>0)
		{
...
// 				$map['encryptedPassword']=md5($pwd);
	...
	public function login($field){
		...
		$user=D('member')->field('uid,username,avatar,sex,nickname,address,sign,mobile')->where(array('username'=>$username,'pwd'=>md5($pwd)))->find();
		//登录
		if(!empty($user)){
			//openfire表中检查数据
			$ofuser=D('Ofuser')->field('username,plainPassword')->where(array('username'=>$username,'plainPassword'=>$pwd))->find();
			if(!empty($ofuser)){
				$user['avatar']=empty($user['avatar'])?dirname(SITE_URL).'/avatar/20131223/20131223165003_55766.jpg':dirname(SITE_URL).'/avatar/'.$user['avatar'];
				$user['sex']=(empty($user['sex'])||($user['sex']==0))?'女':'男';
				$user['nickname']=empty($user['nickname'])?'无':$user['nickname'];
				$user['address']=empty($user['address'])?'无':$user['address'];
				$user['sign']=empty($user['sign'])?'无':$user['sign'];
				$user['mobile']=empty($user['mobile'])?'无':$user['mobile'];
				$this->response->head->res_code="0000";
				$this->response->head->res_msg="success";
				$this->response->body->field=$user;
			}
...
// 		$data['email']=$email;
	...
	public function nearview($field){
		...
		$views=D('Test')->field('id,view_name,view_voice,view_longitude,view_latitude')->where($map)->select();
		if(!empty($views)){
			...
			foreach ($views as $k=>$v){
				...
				if($distance<=50){
					...
					$views[$k]['view_voice']=empty($v['view_voice'])?'':SITE_URL.$v['view_voice'];
