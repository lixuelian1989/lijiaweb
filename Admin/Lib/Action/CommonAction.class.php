...
//            $url = empty($url) ? C('WEB_ROOT') . $_SERVER['REQUEST_URI'] : $url;
	...
	public function upload_pdf($path){
		import("ORG.Net.UploadFile");
		$upload=new UploadFile();
		$upload->maxSize=1024*1024*1024*30;
		$upload->allExts=array("pdf");
	...
	/*public function get_file_content($path=""){
		if($path==''){
			$info=$this->upload_txt();
			if(!$info){
				echo json_encode(array('status' => 0, 'info' =>"上传文件错误！"));
				exit;
			}
