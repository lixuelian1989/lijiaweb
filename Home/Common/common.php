...
/* 
		...
		var suffix     携带后缀...
	*/
function wk_substr($sourcestr, $cutlength, $suffix = true) {
	$returnstr = '';
	$i = 0;
	$n = 0;
	...
	if ($str_length > $cutlength && $suffix) {
		$returnstr = $returnstr . "...";
	}
	return $returnstr;
}
...
function _arctype_parent_getfid($typeid) {
	...
	$arctype = M ( 'arctype' );
	$list = $arctype->where ( $map )->field ( 'fid' )->find ();
	if ($list ['fid'] == 0 or empty ( $list ['fid'] ))
		return 0;
	$map ['id'] = $list ['fid'];
	$list = $arctype->where ( $map )->field ( 'fid' )->find ();
	if ($list ['fid'] == 0 or empty ( $list ['fid'] ))
...
function _page_output($totalnum, $pagesize = 20, $linknum = 5, $option, $getvar = 'p') {
	if ($totalnum < 1 or empty ( $option ) or $pagesize < 1)
		return;
...
function _vote_show($id, $show, $barid, $notitle, $showtotal) {
	...
	if (empty ( $id ) && ! empty ( $_fields ['voteid'] ))
		$id = ( int ) $_fields ['voteid'];
	if ($id < 1)
		return;
	$model = M ( 'vote' );
	$map ['status'] = 1;
	...
	if ($show != 1) {
		...
		if ($notitle != 1)
			$parseStr .= 'bar_' . $barid . '.title = \'' . $list ['title'] . '总计:' . $total . '票\';';
...
function _theme_list($dirname = '', $row = 10) {
	...
	if (! empty ( $dirname )) {
		...
		foreach ( $themearray as $k => $v ) {
			if (! in_array ( $v ['dirname'], $dirname ))
				unset ( $themearray [$k] );
		}
		return $themearray;
	}
	if (count ( $themearray ) > $row)
		return array_slice ( $themearray, 0, $row );
	return $themearray;
}
...
function arctypecount($id = 0, $type = 'son') {
	...
	if ($type == 'top') {
		...
		$fid = $model->where ( 'id=' . $id )->getField ( 'fid' );
		if ($fid == 0)
			return $model->where ( array ('fid' => 0 ) )->count ();
...
/**
	获取费用
	*/
    function get_fee($fee,$guojia){
		if($guojia==1)//中国
			return	sprintf("%.2f元",$fee);
		elseif($guojia==2)//美国
			return sprintf("\$%.2f",$fee);
		else//加拿大
			return sprintf("C\$%.2f",$fee);
